...
class Mxfp4MoEMethod(FusedMoEMethodBase):
    ...
    def __init__(self, moe: FusedMoEConfig):
        ...
        if self.mxfp4_backend == Mxfp4Backend.NONE:
    ...
    def process_weights_after_loading(self, layer):
        if self.mxfp4_backend == Mxfp4Backend.MARLIN:
            ...
            weight_device = layer.w13_weight.device
            layer.w13_weight_scale = Parameter(
                layer.w13_weight_scale.to(device=weight_device,
                                           dtype=torch.uint8),
                requires_grad=False)
            layer.w2_weight_scale = Parameter(
                layer.w2_weight_scale.to(device=weight_device,
                                          dtype=torch.uint8),
                requires_grad=False)
        else:
    ...
    def apply(
        ...
        if self.use_emulated_mxfp4:
            ...
            router_logits_fp16 = (router_logits.to(x_fp16.dtype)
                                  if router_logits.dtype != x_fp16.dtype else
                                  router_logits)
            topk_weights, topk_ids = FusedMoE.select_experts(
                hidden_states=x_fp16,
                router_logits=router_logits_fp16,
                use_grouped_topk=use_grouped_topk,
                top_k=top_k,
                renormalize=renormalize,
                topk_group=topk_group,
